...
void test_group_sstr(lu_test_state *s)
{
    ...
    lu_test_check(s, "NotEmpty", !lu_sstr_empty(&s1));

    lu_sstr_clear(&s1);
    lu_test_check(s, "Clear", !memcmp(s1.str, SSTR_EMPTY.str, LU_SSTRLEN));
    lu_test_check(s, "Empty", lu_sstr_empty(&s1));
    lu_test_check(s, "NotEqual", !lu_sstr_equal(&s1, &s2));
    lu_test_check(s, "NotEqual_0", !lu_sstr_equal_0(&s1, &s2));
    s1 = s2;
    lu_test_check(s, "Equal", lu_sstr_equal(&s1, &s2));
    lu_test_check(s, "Equal_0", lu_sstr_equal_0(&s1, &s2));
    char largestr[64];
    ...
    lu_test_check(s, "StringCopy_0", (bool)strcmp(s2.str, bufcpy));
    lu_sstr_copy_0(&s2, s1.str);
    lu_test_check(s, "StringCopy_0 + Equal", lu_sstr_equal(&s1, &s2));
    lu_sstr s3;
    lu_rng_textg(s3.str, 12);
    lu_sstr_memcpy(&s1, &s3, 235);
    lu_sstr_memcpy_0(&s2, &s3, LU_SSTRLEN);
    lu_test_check(s, "MemCopy", lu_sstr_equal(&s1, &s3));
    lu_test_check(s, "MemCopy_0", lu_sstr_equal(&s2, &s3));
    lu_test_check(s, "MemCopy 2", lu_sstr_equal(&s2, &s1));
    lu_sstr_clear(&s1);
    lu_rng_textg(s1.str, 16);
    s2 = s1;
    lu_rng_textg(s2.str + 7, 5);
    lu_test_check(s, "StringCompare", lu_sstr_cmpstr(&s1, s2.str) == strcmp(s1.str, s2.str));
    lu_test_check(s, "StringCompare_0", lu_sstr_cmpstr_0(&s1, s2.str) == strcmp(s1.str, s2.str));
    lu_test_check(s, "MemoryCompare", lu_sstr_cmpmem(&s1, s2.str, 3) == strncmp(s1.str, s2.str, 3));
    lu_test_check(s, "MemoryCompare_0", lu_sstr_cmpmem_0(&s1, s2.str, 3) == strncmp(s1.str, s2.str, 3));
